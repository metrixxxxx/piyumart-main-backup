  ...
  useEffect(() => {
    async function fetchProduct() {
      ...
      try {
        ...
        // Fetch related using score-based algorithm (category + seller + price)
          ...
          <div className="mt-14">
            <div className="flex items-center gap-3 mb-5">
              <div>
                ...
                <p className="text-xs text-[#1a1060]/40 dark:text-[#f0ede8]/35 mt-0.5">
                  Similar products you might like
                </p>
              </div>
              <div className="flex-1 h-px bg-[#e5e7eb] dark:bg-white/[0.07]" />
              <button
                onClick={() => router.push(`/?category=${product.category_id}`)}
                className="text-xs font-semibold text-[#6d4aff] dark:text-[#c9a96e] hover:opacity-75 transition-opacity shrink-0 bg-transparent border-none cursor-pointer"
              >
                See all →
              </button>
            </div>
