...
export async function PATCH(req, { params }) {
  try {
    ...
    const [orderRows] = await db.query(`SELECT * FROM orders WHERE id = $1`, [orderId]);
    ...
    if (!sellerCheck[0]) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
    ...
    const timestampField = { confirmed: "confirmed_at", shipped: "shipped_at", completed: "completed_at", cancelled: "cancelled_at" }[status];

    let updateFields = [`status = $1`];
    let updateValues = [status];
    let i = 2;

    if (timestampField) { updateFields.push(`${timestampField} = $${i++}`); updateValues.push(now); }
    if (status === "confirmed") { updateFields.push(`seller_confirm_deadline = NULL`); }
    if (status === "shipped" && tracking_number) { updateFields.push(`tracking_number = $${i++}`); updateValues.push(tracking_number); }
    if (status === "shipped" && courier) { updateFields.push(`courier = $${i++}`); updateValues.push(courier); }
    if (status === "cancelled") {
      updateFields.push(`cancel_reason = $${i++}`); updateValues.push(cancel_reason || "seller_unavailable");
      updateFields.push(`cancel_note = $${i++}`); updateValues.push(cancel_note || null);
    }

    updateValues.push(orderId);
    await db.query(`UPDATE orders SET ${updateFields.join(", ")} WHERE id = $${i}`, updateValues);
    ...
    if (BUYER_MESSAGES[status]) {
      await notify({ userId: order.user_id, type: `order_${status}`, message: BUYER_MESSAGES[status] });
    }
    ...
    try {
      await supabaseAdmin
        ...
        .send({ type: "broadcast", event: "orders:updated", payload: broadcastPayload });
    } catch (e) {
    ...
    try {
      await supabaseAdmin
        ...
        .send({ type: "broadcast", event: "orders:updated", payload: { id: orderId, status } });
    } catch (e) {
